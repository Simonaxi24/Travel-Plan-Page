(() => {
  ...
  function elements() {
    return {
      ...
      travelMenu: document.querySelector("#travel-navigation-details"),
      travelHome: document.querySelector("#travel-navigation-home"),
  ...
  function setVisibleView(nextView, options = {}) {
    const { travelView, ledgerView, travelHome, ledgerLink, skipLink } = elements();
    ...
    if (travelHome) {
      if (ledgerActive) travelHome.removeAttribute("aria-current");
      else travelHome.setAttribute("aria-current", "page");
    }
    if (ledgerLink) {
  ...
  function setup() {
    ...
    document.addEventListener("click", (event) => {
      ...
      const travelLink = event.target.closest(".travel-navigation-menu a, #travel-navigation-home, #wordmark");
      if (travelLink) {
        event.preventDefault();
        travelMenu?.removeAttribute("open");
