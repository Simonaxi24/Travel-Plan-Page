...
function mapRouteDefinitions(source) {
  const seen = new Set();
  return routeLayersFor(source).filter((route) => {
    if (seen.has(route.day)) return false;
    seen.add(route.day);
    return true;
  }).map(({ day, color }) => ({ day, color }));
}
...
function mapArtwork(source, selected, id, viewport) {
  ...
  svg.querySelectorAll('g[id^="overview-route-"]').forEach((group) => {
    if (group.dataset.day !== String(selected.day)) group.remove();
  });
...
function renderRoutePanel(regionId, dayNumber = 0) {
  ...
  root.innerHTML = `<div class="route-region-tabs" aria-label="地图区域">${regions.map((region) => `<button type="button" data-route-region="${escapeHtml(region.id)}" aria-pressed="${region.id === source.id}">${escapeHtml(region.label || region.heading?.text || region.id)}</button>`).join("")}</div>
