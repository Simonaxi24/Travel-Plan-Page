...
function labelFor(place, index, occupied) {
  ...
  const width = Math.max(154, Math.min(600, Math.max(String(place.name || "").length, String(place.nameZh || "").length * 2) * 13));
  let best;
...
function mapDataForRegion(mapData, region, regionCount) {
  ...
  return { ...mapData, templateId: region.templateId || mapData.templateId, region, places, routes, dailyRoutes };
}
...
function buildRegion(mapData, manifest) {
  ...
  const routes = mapData.routes.map((route) => {
    ...
    return { day: route.day, mode: route.mode || "route", color: GOLDEN.routeColors[(route.day - 1) % GOLDEN.routeColors.length], placeIds: ids, paths: detailed ? [detailed] : [], overviewPaths: overview ? [overview] : [] };
  });
  const dailyDefinitions = mapData.dailyRoutes?.length ? mapData.dailyRoutes : mapData.routes;
  const dailyLayouts = {};
  dailyDefinitions.forEach((daily) => {
    ...
    const key = String(daily.day);
    const previous = dailyLayouts[key] || { places: [], labels: {}, transport: [] };
    dailyLayouts[key] = {
      places: [...new Set([...previous.places, ...uniqueIds])],
      labels: { ...previous.labels, ...labels },
      transport: [...previous.transport, ...transport]
    };
  });
