...
function travelOverviewArtwork(days, source = {}, options = {}) {
  ...
  const legendRoutes = routeLayers.filter((route, index, routes) => routes.findIndex((candidate) => candidate.day === route.day) === index);
  ...
  const pathClass = (route) => route.mode === "flight" ? "overview-flight-path" : "overview-ground-path";
  const routeIcon = (route, path) => route.mode === "flight"
    ? `<text class="overview-plane-icon" x="50%" y="50%"><textPath href="#${esc(pathId(route, path))}" startOffset="50%">✈</textPath></text>`
    : "";
  const pathId = (route, path) => `overview-route-path-${esc(source.id || "map")}-${esc(route.day)}-${Math.abs(hashString(path))}`;
  const esc = (value) => String(value).replace(/[&<>"']/g, (character) => ({
    "&": "&amp;", "<": "&lt;", ">": "&gt;", "\"": "&quot;", "'": "&apos;"
  })[character]);
  const hashString = (value) => String(value).split("").reduce((hash, character) => ((hash << 5) - hash + character.charCodeAt(0)) | 0, 0);
  ...
  ${routeLayers.map((route, routeIndex) => `<g id="overview-route-${route.day}-${routeIndex}" class="${pathClass(route)}" inkscape:groupmode="layer" inkscape:label="${esc(days.find((day) => day.day === route.day)?.date || route.day)}" data-day="${route.day}" data-date="${esc(days.find((day) => day.day === route.day)?.date || "")}" fill="none" stroke-linecap="round" stroke-linejoin="round">${pathsFor(route).map((path) => { const id = pathId(route, path); return `<path class="overview-route-halo" d="${esc(path)}" stroke="#fffaf0" stroke-opacity=".78" stroke-width="13"/><path class="overview-route-main" id="${id}" d="${esc(path)}" stroke="${esc(route.color)}" stroke-width="8"/><path class="overview-route-highlight" d="${esc(path)}" stroke="#ffffff" stroke-opacity=".28" stroke-width="2"/>${routeIcon(route, path)}`; }).join("")}</g>`).join("")}
  <g id="overview-markers" inkscape:groupmode="layer" inkscape:label="地点标记">${placeLayers.map((place, index) => `<g id="overview-point-${esc(place.id)}"><circle cx="${place.x}" cy="${place.y}" r="16" fill="${esc(place.color)}" stroke="#fffaf0" stroke-width="3"/><text x="${place.x}" y="${place.y + .5}" fill="#fffaf0" font-size="13" font-weight="800" text-anchor="middle" dominant-baseline="middle" font-family="ui-monospace, SFMono-Regular, Menlo, monospace">${index + 1}</text></g>`).join("")}</g>
  <g id="overview-place-names" inkscape:groupmode="layer" inkscape:label="地点名称" fill="#092653" stroke="#f8f4ea" stroke-width="3.5" stroke-linejoin="round" paint-order="stroke fill" font-family="'Times New Roman', 'Kaiti SC', STKaiti, KaiTi, 'Songti SC', serif" font-weight="700">${placeLayers.map((place) => `<text id="overview-label-${esc(place.id)}" x="${place.tx}" y="${place.ty}" text-anchor="${esc(place.anchor || "start")}" font-size="${options.useDetailedRoutes ? Number(place.size) || 24 : Math.max(28, Number(place.size) || 24)}">${multilineText(place.lines, place.tx, options.useDetailedRoutes ? 31 : 34)}</text>`).join("")}</g>
